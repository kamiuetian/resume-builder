...
const LoginModal = ({ isOpen, setShowLoginModal, setShowSignUpModal }: Props) => {
    ...
    async function createNewAccount(e: FormType) {
        ...
        if (json.status === "success" && json.codeSent) {
            ...
            setCredentials({ email: e.email, password: e.password });
        }
    ...
    async function verifyEmail() {
        ...
        const res = await fetch("/api/auth/verify-email", {
            ...
            body: JSON.stringify({ code: verificationCode, email: credentials.email }),
        });


        const { status, message } = await res.json();
        if (status === "success") {
            const signin = await signIn('credentials', { email: credentials.email, password: credentials.password, redirect: false });
    ...
    if (stepOneCompleted) {
        return <div className="fixed inset-0 z-50 overflow-auto bg-white bg-opacity-50 flex backdrop-blur-sm">
            {/* Modal */}
            <div className="relative p-6 bg-white w-fit max-w-lg m-auto flex-col flex">
                {/* Content */}
                <div className="modal-content">
                    <div className=''>
                        <div className='w-full'>
                            <h2 className="text-primary text-center pb-2 text-4xl font-bold lg:text-5xl whitespace-nowrap">
                                Email verification
                            </h2>
                                ...
                                <div className="w-96 flex justify-between mt-6">

                                    {
                                        verificationProgress ?
                                            <div
                                                className="mx-auto bg-primary cursor-pointer px-8 py-2 font-semibold border-2 opacity-70 text-white rounded text-lg"
                                            >
                                            ...
                                            <div
                                                onClick={verifyEmail}
                                                className="mx-auto bg-primary cursor-pointer px-8 py-2 font-semibold border-2  text-white rounded text-lg"
                                            >
                                                Verify
                                            </div>
                                    }
                ...
                <div className="modal-content">
                    <div className=''>
                        <div className='w-fit'>
                            ...
                            <h2 className="text-primary text-center cursor-default pb-2 text-4xl font-bold lg:text-5xl whitespace-nowrap">
                                Create new account
                            </h2>

                            <div>
                                <p className="text-lg cursor-default font-medium text-gray-400 text-center mt-3">
                                ...
                                <div className="flex flex-col items-center">
                                    ...
                                    <form
                                        ...
                                        <div className='flex flex-col mx-auto lg:w-96 md:w-80 w-72 '>
                                            ...
                                            <input
                                                onKeyDown={() => setPasswordMatchError(false)}
                                        ...
                                        <div className='flex flex-col mx-auto lg:w-96 md:w-80 w-72'>
                                            ...
                                            <input
                                                ...
                                                type="password" className='bg-gray-100 px-3 py-2 text-lg outline-none border-b-2 border-b-transparent focus:border-b-2 focus:border-b-blue-500' />
                                            {passwordMatchError && <span className='text-sm font-semibold text-red-500'>Password does not match</span>}
                                        </div>
                                        <div className='flex flex-col mx-auto lg:w-96 md:w-80 w-72 '>
                                        ...
                                        <div className='flex flex-col mx-auto lg:w-96 md:w-80 w-72 '>
                                            <p className='text-gray-500 font-semibold'>Already have an account? Click<span onClick={showLoginModal} className='text-blue-500 cursor-pointer hover:text-blue-600'> here</span></p>
                                        </div>
                                        <div className='flex justify-center'>

                                            {
                                                signUpProgress ?
                                                    ...
                                                    <button disabled type="button" className="opacity-70 bg-primary w-fit mx-auto px-12 py-2 font-semibold border-2 border-blue-500 text-white rounded-md text-lg">
                                                        <svg aria-hidden="true" role="status" className="inline w-4 h-4 mr-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB" />
                                                            <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor" />
                                                        </svg>
                                                    ...
                                                    <button
                                                        type='submit'
                                                        className="bg-primary w-fit mx-auto px-12 py-2 font-semibold border-2 border-blue-500 text-white rounded-md text-lg">Continue</button>
                                            }
